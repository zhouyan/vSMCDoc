...
To access $X_{ij}$, the value of the state of the $i$\ith particle at
coordinate $j$, one can use the method \cppinline{sm.state(i, j)}. The method
\cppinline{sm.data()} returns a pointer to the beginning of the matrix. If
\cppinline{Layout} is \cppinline{RowMajor}, then the method
\cppinline{sm.row_data(i)} returns a pointer to the beginning of the $i$\ith
row. If \cppinline{Layout} is \cppinline{ColMajor}, then the method
\cppinline{sm.col_data(j)} returns a pointer to the beginning of the $j$\ith
...
example of changing size of a sampler is provided in section~\ref{sub:Resizing
  a sampler}.
...
object\footnote{More precisely, it is a \cppinline{Particle<T>::weight_type}
  ...
  section~\ref{sub:Customizing member types} for more details. If the user does
...
\rng system are documented later in section~\ref{sec:Random number generating}.
...
\end{cppcode}
Details can be found in section~\ref{sub:Storing objects in HDF5}.
...
The complete program is shown in appendix~\ref{sub:Sequential implementation of
  a simple particle filter}. In this section we show the outline of the
implementation.
...
it is possible and an example is shown in section~\ref{sub:Resizing a sampler}.
The column \routinline{Resampled} shows nonzero values if resampling were
...
column \routinline{ESS} shows the value of \ess. The last two columns show the
...
system in \cppinline{eval_post}. Later in section~\ref{sub:Symmetric
...
appendix~\ref{sub:Parallelized implementation of a simple particle filter}.
