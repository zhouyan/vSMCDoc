...
\cppinline{NormalMVDistribution} in section~\ref{sub:Distributions} accepts
...
appendix~\ref{sub:Processing command line program options}. First, one need to
...
supports standard library \io operations is also supported. Then one constructs
a \cppinline{ProgramOptionMap} object,
\begin{cppcode}
  ProgramOptionMap option_map;
\end{cppcode}
Options can be added to the map,
\begin{cppcode}
  option_map
...
a minimal example. The complete program is shown in appendix~\ref{sub:Display
  program progress}.
\begin{cppcode}
