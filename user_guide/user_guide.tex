...
section~\ref{sec:Resample}. Last, one can also construct a sampler that is only
resampled when $\ess < \alpha N$, $\alpha\in[0, 1]$, by the following,
\begin{cppcode}
...
it is possible and an example is shown in section~\ref{sec:Resample}. The
...
\label{ssub:Initialization: PFInit}
...
\subsection{Customizing \protect\texttt{SingleParticle<T>}}
\label{sub:Customizing SingleParticle}

\subsection{Customizing \protect\texttt{Particle<T>} member types}
\label{sub:Customizing Particle<T> member types}
...
$s \equiv r \mathrm{mode} n$. Therefore, no two nodes will ever generate the
same seeds.
...
is most significant development for parallel Monte Carlo algorithms in recent
memory. See the paper for more details. Here, it is sufficient to mention that,
the \rng introduce in the paper use a deterministic function $f_k$, such that,
for any sequence $\{c_i\}_{i>0}$, the sequence $\{y_i\}_{i>0}$, $y_i =
f_k(c_i)$, appears as random. In addition, for $k_1 \ne k_2$, $f_{k_1}$ and
$f_{k_2}$ will generate two sequences that appear statically independent.
Compared to more conventional \rng{}s which use recursions $y_i = f(y_{i -
  1})$, these counter-based \rng{}s are much easier to setup in a parallelized
environment.

If $c$, the counter, is an unsigned integers with $b$ bits, and $k$, the key,
is an unsigned integer with $d$ bits. Then for each $k$, the \rng has a period
...
\textcite{Salmon:2011um} are implemented with a few additions. Note that, the
actual period of an \rng can be longer. For example, \cppinline{Philox4x64} has
a 256-bits counter and but output 64-bits integers. And thus it has a
...
Note that, note all \rng{}s defined by the library is available on all
...
\begin{cppcode}
  using RNG = ARS_4x32;
\end{cppcode}
otherwise if \avx2 instructions are supported,
\begin{cppcode}
  using RNG = Threefry4x32AVX2;
\end{cppcode}
otherwise if \sse2 instructions are supported,
\begin{cppcode}
...
\end{cppcode}
This can be changed by the configurations macro \cppinline{VSMC_RNG_TYPE}.
...
The output 16-, 32-, and 64-bits random integers, respectively.
...
a wrapper class \cppinline{MKLEngine} that make them accessible as \cppoo{}
...
bigger state space them usual \rng{}s.
...
\cppinline{particle.rng(i)} returns independent \rng instances. This is
...
interface is required while the thread-safety and other issues is not
important.
...
\end{cppcode}
The type template alias \cppinline{RNGSet} is defined to be
\cppinline{RNGSetTBB} if \tbb is available. Otherwise it is defined to be
\cppinline{RNGSetVector}. It is used by the \cppinline{Particle} class
template. One can replace the type of \rng set used by \cppinline{Particle<T>}
with a member type of \cppinline{T}. For example,
\begin{cppcode}
  class T
  {
