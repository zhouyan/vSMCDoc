...
\href{https://github.com/zhouyan/vSMC/releases}{Releases} or the development
branch from the Git repository. This is a header only template \cpp library. To
install the library just move the contents of the \shinline{include} directory
into a proper place, e.g., \shinline{/usr/local/include} on Unix-alike systems.
Alternatively, one can use \cmake (version 2.8.3 or later required).
\begin{shcode}
